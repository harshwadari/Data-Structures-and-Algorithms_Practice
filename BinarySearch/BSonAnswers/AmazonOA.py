...
As a logistics manager in an automobile manufacturing company, you are responsible 
for storing deliveries in secure warehouses.

You are given an integer array `deliveryLogs` of size `n`, where each element 
represents the number of parts delivered in the `i`-th encrypted delivery log.
...
* After storing, exactly `k/2` warehouses having the **largest number of d
eliveries** become compromised.
...
The important constraint for understanding the problem is:

> **A single log can be split among multiple warehouses, but one warehouse cannot contain deliveries from multiple logs.**

That's the part that makes this problem different from straightforward partitioning.

"""


"""
Split the delivery logs into exactly k warehouses, while keeping each warehouse 
associated with only one log, and distribute the deliveries as evenly as possible so 
that after the largest k/2 warehouses are compromised, the total deliveries remaining in 
the safe warehouses is maximized.
"""
